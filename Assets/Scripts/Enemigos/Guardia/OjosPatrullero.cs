...
public class OjosPatrullero : MonoBehaviour
{
    ...
    [Header("Detecci�n Visual de Objetos")]
    ...
    // Guardamos la m�scara de colisi�n para no recalcularla en cada frame
    ...
    void DetectarJugadorYObjetos()
    {
        ...
        foreach (Collider cosa in cosasCercanas)
        {
            ...
            if (Vector3.Angle(transform.forward, direccionHaciaCosa) <= anguloVision / 2f)
            {
                ...
                {
                    // NUEVO: Le pasamos la m�scara y le decimos que ignore los Triggers
    ...
    void DibujarCono()
    {
        ...
        for (int i = 0; i < resolucion; i++)
        {
            ...
            // NUEVO: Usamos la m�scara para dibujar, ignorando los Triggers
